...
If Not Cartel Then
    ...
    i = 0
    Do While LeyendaFormateada(i) <> vbNullString And i < UBound(LeyendaFormateada) ' GSZ
...
If anti + i <= Len(s) + 1 Then
    If ((i >= MAXLONG) And mid$(s, anti + i, 1) = " ") Or (anti + i = Len(s)) Then
        LeyendaFormateada(k) = mid$(s, anti, i + 1) ' GSZ
        k = k + 1
        anti = anti + i + 1
        i = 0
    Else
...
For j = 0 To UBound(LeyendaFormateada)
    Engine.Fonts_Render 1, X, Y + desp, X + 200, Y + desp + 300, LeyendaFormateada(j), D3DColorXRGB(255, 255, 255)
